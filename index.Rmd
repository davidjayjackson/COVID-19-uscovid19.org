...
# ggplotly(cases_1)
 ...
 US %>% filter(Reported >= Sys.Date() - 60) %>%  ggplot() + geom_col(aes(x=Reported,y=Deaths,fill=Deaths)) +
  theme(axis.text.x = element_text(angle = 45)) +
...
US %>% ggplot(aes(x=Reported,y=Cases)) + geom_line() +
  ...
  labs(title="US Cases by Date(Log10)",x="Date",y=" Log of Cases")
```
```{r}
US %>% ggplot(aes(x=Reported,y=Cases)) + geom_line() +
  geom_smooth() + 
  labs(title="US Cases by Date",x="Date",y=" Log of Cases")
```
