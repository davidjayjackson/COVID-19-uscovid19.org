...
ggplot(df) + geom_line(aes(x=date,y=new_cases,col=Year)) +
  labs(title = "Daily Cases: 2020 vs 2021")
ggplot(df) + geom_line(aes(x=date,y=new_deaths,col=Year)) +
...
```{r}
df %>% group_by(date) %>%
  summarise_by_time(date,.by = "quarter",Cases =sum(new_cases)) %>% ggplot(aes(x=date,y=Cases)) + geom_line() +
  labs(title = "Cases by Quarter")

df %>% group_by(date) %>%
  summarise_by_time(date,.by = "quarter",Deaths =sum(new_deaths)) %>% ggplot(aes(x=date,y=Deaths)) + geom_line() +
