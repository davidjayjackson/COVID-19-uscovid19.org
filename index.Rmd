...
df <- df %>%select(date,location,iso_code,new_cases,new_deaths,new_vaccinations,new_vaccinations_smoothed,total_vaccinations,people_vaccinated,people_fully_vaccinated,population)
...
```{r}
p7 <- ggplot(vac) + geom_line(aes(x=date,y=people_vaccinated)) +
  scale_y_continuous(labels=comma) +
  labs(title="US's People Vaccinated By Date") +
  geom_smooth(aes(x=date,y=new_vaccinations))
```
```{r}
p8 <- ggplot(vac) + geom_line(aes(x=date,y=people_fully_vaccinated)) +
  scale_y_continuous(labels=comma) +
  labs(title="US's People Fully Vaccinated By Date") 
...
p9 <- ggplot(df) + geom_line(aes(x=date,y=Rate)) + 
    scale_y_continuous(labels=percent) +
  labs(title="US's vaccination Rate By Day")
ggplotly(p5)
ggplotly(p6)
ggplotly(p7)
ggplotly(p8)
